...
A migração `db/migrations/021_remove_cancelled_budgets.sql` exclui orçamentos antigos com status `CANCELADO` que não tenham venda nem movimento financeiro/estoque vinculado. Antes de aplicá-la em produção, confirme o backup e execute `npm run migrate -- --dry-run`; depois aplique as pendências com `npm run migrate`. Orçamentos cancelados daqui em diante são excluídos diretamente pela aplicação, com confirmação e registro na auditoria. Pedidos e vendas não podem ser excluídos por esse botão.
