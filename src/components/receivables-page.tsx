...
type Receivable = { id: string; workOrderId?: string; orderNumber?: number; customerId?: string; customerName: string; dueDate?: string; paymentDate?: string; amount: number; openAmount: number; status: Status; paymentMethod?: string; installmentNumber:number;installmentCount:number;notes?: string };
type Customer = { id: string; name: string };
type Order = { id: string; number: number; customerId: string; customerName: string; total: number };
type Account={id:string;name:string;type:string};type FeeRule={minimumInstallments:number;maximumInstallments:number|null;feePercent:number};
type PaymentMethod={id:string;name:string;supportsInstallments:boolean;variableFee:boolean;defaultFeePercent:number;defaultAccountId?:string;rules:FeeRule[]};
type Payload = { receivables: Receivable[]; customers: Customer[]; orders: Order[];accounts:Account[];paymentMethods:PaymentMethod[] };
...
export function ReceivablesPage({ reconciliation = false }: { reconciliation?: boolean }) {
  const [data, setData] = useState<Payload>({ receivables: [], customers: [], orders: [],accounts:[],paymentMethods:[] });
  ...
  const selectedMethod=data.paymentMethods.find(item=>item.id===paymentMethodId);
  const feePercent=selectedMethod?(selectedMethod.variableFee?(selectedMethod.rules.find(rule=>(paying?.installmentCount??1)>=rule.minimumInstallments&&(rule.maximumInstallments===null||(paying?.installmentCount??1)<=rule.maximumInstallments))?.feePercent??selectedMethod.defaultFeePercent):selectedMethod.defaultFeePercent):0;
  const feeAmount=(paying?.openAmount??0)*feePercent/100;
  ...
  async function change(item: Receivable, action: "PAY" | "REOPEN") {
    ...
    try {
      const response = await fetch(`/api/receivables/${item.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, paymentDate, paymentMethod, paymentMethodId,financialAccountId }) });
