...
type Receivable = { id: string; workOrderId?: string; orderNumber?: number; customerId?: string; customerName: string; dueDate?: string; paymentDate?: string; amount: number; status: Status; paymentMethod?: string; notes?: string };
...
export function ReceivablesPage({ reconciliation = false }: { reconciliation?: boolean }) {
  ...
  return <div className="app-shell"><AppSidebar active={reconciliation ? "conferencia" : "receber"} /><main className="workspace registry-workspace">
    ...
    <section className="operations-stats finance-stats"><div><CircleDollarSign /><span>Em aberto</span><strong>{money.format(open.reduce((sum, item) => sum + item.amount, 0))}</strong></div><div><AlertTriangle /><span>Vencido</span><strong>{money.format(overdue.reduce((sum, item) => sum + item.amount, 0))}</strong></div><div><CalendarClock /><span>Vence hoje</span><strong>{money.format(dueToday.reduce((sum, item) => sum + item.amount, 0))}</strong></div><div><BadgeCheck /><span>Recebido</span><strong>{money.format(paid.reduce((sum, item) => sum + item.amount, 0))}</strong></div></section>
