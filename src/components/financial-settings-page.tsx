...
type Account = {
  id: string;
  name: string;
  type: "CAIXA" | "BANCO" | "CARTEIRA" | "OUTRA";
...
type Editing = { entity: "account" | "method"; id?: string };
const accountLabels = {
  CAIXA: "Caixa",
  BANCO: "Banco",
...
export function FinancialSettingsPage() {
  ...
  const [accountType, setAccountType] = useState<Account["type"]>("BANCO");
  ...
  const [rules, setRules] = useState<FeeRule[]>([blankRule()]);
  async function load(signal?: AbortSignal) {
    const response = await fetch("/api/financial-settings", { signal });
    const payload = await response.json();
    if (!response.ok)
      throw new Error(
        payload.error ?? "Não foi possível carregar as configurações.",
      );
  ...
  useEffect(() => {
    ...
    fetch("/api/financial-settings", { signal: controller.signal })
      .then(async (response) => {
        const payload = await response.json();
        if (!response.ok)
          throw new Error(
  ...
  function openAccount(account?: Account) {
    setEditing({ entity: "account", id: account?.id });
    setName(account?.name ?? "");
    setAccountType(account?.type ?? "BANCO");
    setOpeningBalance(numberToMoneyInput(account?.openingBalance ?? 0));
  ...
  function openMethod(method?: Method) {
    ...
    setRules(method?.rules.length ? method.rules : [blankRule()]);
    setError("");
  }
  async function save(event: FormEvent) {
    ...
    try {
      ...
      const response = await fetch(
        editing.id
          ? `/api/financial-settings/${editing.entity}/${editing.id}`
      ...
      setNotice(
        `${editing.entity === "account" ? "Conta" : "Forma de pagamento"} ${editing.id ? "atualizada" : "cadastrada"}.`,
      );
    } catch (caught) {
      setError(
  ...
  return (
    <div className="app-shell">
      <AppSidebar active="configuracoes_financeiras" />
      <main className="workspace registry-workspace">
        <header className="topbar registry-topbar">
          ...
          <div>
            <p className="eyebrow">CONFIGURAÇÕES FINANCEIRAS</p>
            <h1>Contas e taxas</h1>
            <p className="page-description">
              Defina onde o dinheiro entra e as taxas de cada forma de
              pagamento.
            </p>
          </div>
        </header>
        {mobileMenu && (
          <div className="mobile-shortcuts registry-shortcuts">
            ...
            <Link href="/categorias-financeiras">Categorias</Link>
          </div>
        )}
        <section className="operations-stats">
          <div>
            ...
            <strong>{accounts.filter((item) => item.active).length}</strong>
          </div>
          <div>
            <CreditCard />
            <span>Formas ativas</span>
          ...
          <div>
            ...
            <strong>
              {methods.filter((item) => item.active && item.variableFee).length}
            </strong>
          </div>
        </section>
        {loading ? (
          <section className="registry-panel registry-loading">
          ...
          <div className="settings-columns">
            <section className="registry-panel">
              <div className="operations-title">
                <div>
                  <span className="section-kicker">TESOURARIA</span>
                  <h2>Contas financeiras</h2>
                </div>
                <button className="table-action" onClick={() => openAccount()}>
                  <CirclePlus size={16} />
                  Nova conta
                </button>
              </div>
                ...
                <table className="registry-table">
                  ...
                  <tbody>
                    {accounts.map((account) => (
                      <tr
                        key={account.id}
                        className={!account.active ? "inactive-row" : ""}
                      >
                        <td>
                          <strong>{account.name}</strong>
                        </td>
                        <td>{accountLabels[account.type]}</td>
                        <td>
                          <strong>
            ...
            <section className="registry-panel">
              <div className="operations-title">
                ...
                <button className="table-action" onClick={() => openMethod()}>
                  <CirclePlus size={16} />
                  Nova forma
                </button>
              </div>
                ...
                <table className="registry-table">
                  ...
                  <tbody>
                    {methods.map((method) => (
                      <tr
                        key={method.id}
                        className={!method.active ? "inactive-row" : ""}
                      >
                        <td>
                        ...
                        <td>
                          {method.variableFee
                            ? `${method.rules.length} faixa(s)`
                            : `${method.defaultFeePercent.toLocaleString("pt-BR")}%`}
                        </td>
                        <td>
                          {accounts.find(
                            (account) => account.id === method.defaultAccountId,
                          )?.name ?? "—"}
                        </td>
                        <td>
                          <button
            ...
            <header className="modal-header">
              <div>
                <span className="section-kicker">
                  {editing.entity === "account" ? "TESOURARIA" : "PAGAMENTOS"}
                </span>
                <h2>
                  {editing.id ? "Editar" : "Cadastrar"}{" "}
                  {editing.entity === "account"
                    ? "conta"
                    : "forma de pagamento"}
                </h2>
              </div>
                ...
                {editing.entity === "account" ? (
                  <>
                    ...
                    <label className="field">
                      ...
                      <MoneyInput value={openingBalance} onValueChange={setOpeningBalance} allowNegative />
                    </label>
                  </>
                ) : (
                  <>
                    <label className="field">
                      ...
                      >
                        ...
                        {accounts
                          .filter(
                            (item) =>
                              item.active || item.id === defaultAccountId,
                          )
                          .map((item) => (
                    ...
                    <label className="checkbox-field">
                      <input
                        ...
                        onChange={(event) =>
                          setSupportsInstallments(event.target.checked)
                        }
                      />
                    ...
                    <label className="checkbox-field">
                      <input
                        ...
                        checked={variableFee}
                        onChange={(event) =>
                          setVariableFee(event.target.checked)
                        }
                      />
                      <span>Usar taxa variável por parcelas</span>
                    </label>
                    {variableFee && (
                      <fieldset className="fee-rules span-2">
                        <legend>Faixas de taxa</legend>
