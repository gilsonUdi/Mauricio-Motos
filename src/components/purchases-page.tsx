...
export function PurchasesPage() {
  ...
  const total = useMemo(() => items.reduce((sum, item) => sum + (Number(item.quantity.replace(",", ".")) || 0) * parseBrazilianNumber(item.unitCost), 0), [items]);
  ...
  async function save(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/purchases", {
        ...
        body: JSON.stringify({ supplierId:supplierId||undefined,supplier, date, items: items.map((item) => ({ productId: item.productId, quantity: Number(item.quantity.replace(",", ".")), unitCost: parseBrazilianNumber(item.unitCost) })) }),
      });
      const payload = await response.json();
