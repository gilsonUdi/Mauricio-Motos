...
export function FinancePage() {
  ...
  async function save(event: FormEvent) {
    ...
    try { const selected=data?.availableCategories.find(item=>item.id===category);const response = await fetch("/api/finance", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ date, description, movement, categoryId:category||undefined,category:selected?.name, amount: parseBrazilianNumber(amount) }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error ?? "Não foi possível salvar."); await load(); setShowForm(false); setNotice("Lançamento financeiro registrado."); }
