...
type FinanceData = { summary: { income: number; expense: number; result: number; openAmount: number; overdueAmount: number; overdueCount: number }; transactions: Transaction[]; categories: Category[] };
...
export function FinancePage() {
  ...
  const [movement, setMovement] = useState<"ENTRADA" | "SAIDA">("SAIDA"); const [category, setCategory] = useState("Despesas gerais"); const [amount, setAmount] = useState("");
  ...
  {showForm && <div className="modal-backdrop"><section className="registry-modal"><header className="modal-header"><div><span className="section-kicker">FINANCEIRO</span><h2>Novo lançamento</h2></div><button className="icon-button" onClick={() => setShowForm(false)}><X /></button></header><form className="registry-form" onSubmit={save}><div className="form-grid registry-form-grid"><label className="field"><span>Data *</span><input type="date" value={date} onChange={(event) => setDate(event.target.value)} required /></label><label className="field"><span>Movimento *</span><select value={movement} onChange={(event) => setMovement(event.target.value as "ENTRADA" | "SAIDA")}><option value="ENTRADA">Entrada</option><option value="SAIDA">Saída</option></select></label><label className="field span-2"><span>Descrição *</span><input value={description} onChange={(event) => setDescription(event.target.value)} required /></label><label className="field"><span>Categoria *</span><input value={category} onChange={(event) => setCategory(event.target.value)} required /></label><label className="field"><span>Valor *</span><input type="number" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} required /></label></div>{error && <p className="form-error">{error}</p>}<footer className="modal-actions"><button type="button" className="secondary-button" onClick={() => setShowForm(false)}>Cancelar</button><button className="primary-button" disabled={saving}>{saving ? <LoaderCircle className="spin" size={18} /> : <Plus size={18} />}Salvar lançamento</button></footer></form></section></div>}
