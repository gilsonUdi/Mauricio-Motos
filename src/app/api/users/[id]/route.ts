...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try { await ensureAuthSchema(); const pool = getPool()!; const result = await pool.query(`UPDATE app_live.app_users SET name=$2,email=$3,role=$4,permissions=$5::text[],active=$6,password_hash=CASE WHEN $7='' THEN password_hash ELSE crypt($7,gen_salt('bf',12)) END,updated_at=now() WHERE id=$1::uuid RETURNING id`, [id,name,email,role,role === "ADMIN" ? [...permissionIds] : permissions,active,password]); if (!result.rowCount) return NextResponse.json({ error: "Usuário não encontrado." }, { status: 404 }); await pool.query(`INSERT INTO app_live.audit_log(entity_type,entity_id,action,actor_id,details) VALUES('app_user',$1::uuid,'updated',$2::uuid,$3::jsonb)`, [id,actor.id,JSON.stringify({name,email,role,permissions,active,passwordChanged:Boolean(password)})]); return NextResponse.json({ ok: true }); }
