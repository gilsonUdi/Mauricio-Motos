...
export async function GET() {
  ...
  try { await ensureAuthSchema(); const rows = await getPool()!.query(`SELECT id::text,name,email,role,permissions,active,last_login_at,created_at FROM app_live.app_users WHERE company_id=$1::uuid ORDER BY active DESC,name`,[actor.companyId]); return NextResponse.json({ users: rows.rows }); }
