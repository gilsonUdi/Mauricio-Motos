...
export async function PATCH(request:Request,context:{params:Promise<{entity:string;id:string}>}){
  ...
  try{
    ...
    if(entity==="account"){
      ...
      const supportsInstallments=body.supportsInstallments===true;const maximumInstallments=supportsInstallments?Math.max(1,Math.min(120,Math.trunc(Number(body.maximumInstallments)||1))):1;
      const variableFee=body.variableFee===true;const defaultFee=Number(body.defaultFeePercent)||0;const rules=Array.isArray(body.rules)?body.rules as FeeRule[]:[];
      ...
      result=await client.query(`UPDATE app_live.payment_methods SET name=$2,supports_installments=$3,maximum_installments=$4,variable_fee=$5,default_fee_percent=$6,default_account_id=$7::uuid,active=$8,updated_at=now() WHERE id=$1::uuid AND company_id=$9::uuid RETURNING id`,[id,name,supportsInstallments,maximumInstallments,variableFee,defaultFee,defaultAccountId,body.active!==false,scope.companyId]);
