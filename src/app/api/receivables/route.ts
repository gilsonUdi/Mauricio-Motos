  ...
  try {
    const [receivables, customers, orders,accounts,methods,rules] = await Promise.all([
      pool.query(`
        SELECT r.id::text, r.work_order_id::text, o.order_number, r.customer_id::text,
               ...
               COALESCE(last_payment.payment_method,pm.name,o.payment_method) AS payment_method,r.installment_number,r.installment_count,
        ...
        LEFT JOIN app_live.payment_methods pm ON pm.id=r.payment_method_id
    ...
    return NextResponse.json({
      receivables: receivables.rows.map((row) => ({
        ...
        paymentMethod: row.payment_method, paymentMethodId:row.payment_method_id, financialAccountId:row.financial_account_id,
        feePercent:Number(row.fee_percent??0), installmentNumber:Number(row.installment_number??1),installmentCount:Number(row.installment_count??1), notes: row.notes,
      })),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO app_live.audit_log (entity_type, entity_id, action, actor_id, details) VALUES ('receivable', $1::uuid, 'created', $2::uuid, $3::jsonb)`,
      [inserted.rows[0].id, scope.user?.id ?? null, JSON.stringify({ customerName, amount, dueDate, workOrderId, companyId: scope.companyId })],
    );
